...
def tour_active(request):
    ...
    progress_map = {
        p.tour_id: p
        for p in TourProgress.objects.filter(user=user, tour__in=tours)
    }
...
def tour_progress(request):
    ...
    progress, _created = TourProgress.objects.get_or_create(tour=tour, user=request.user)
...
def tour_step_form(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            obj.save()
            messages.success(
                request, _("Step updated") if step else _("Step added")
            )
